...
fn read_json_dataset(path: &str) -> Result<Dataset, Box<dyn std::error::Error>> {
...
fn main() {
    ...
    let old_dataset = match read_json_dataset(&args[1]) {
        Ok(ds) => ds,
        Err(e) => {
    ...
    let new_dataset = match read_json_dataset(&args[2]) {
        Ok(ds) => ds,
        Err(e) => {
