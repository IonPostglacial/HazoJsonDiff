...
pub fn diff_json_strs(old_json: &str, new_json: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    for prop in ["taxons", "characters", "states", "books"] {
        let old_p = get_prop(&old_val, prop).ok_or_else(|| format!("property '{prop}' missing in old JSON"))?;
        let new_p = get_prop(&new_val, prop).ok_or_else(|| format!("property '{prop}' missing in new JSON"))?;
